...
function toDown(id) {
  ...
  if (keys[id].inner) {
    ...
    switch (id) {
      case 'Backspace': {
        const BackspaceSelStart = textarea.selectionStart;
      ...
      case 'Delete': {
        const DeleteSelEnd = textarea.selectionEnd;
        if (DeleteSelEnd !== textarea.value.length) {
          textarea.value = textarea.value.substring(0, textarea.selectionStart)
            + textarea.value.substring(textarea.selectionEnd + 1);
          textarea.setSelectionRange(DeleteSelEnd, DeleteSelEnd);
        }
        break;
      }
      case 'Enter': {
        const EnterSelEnd = textarea.selectionEnd;
        textarea.value = `${textarea.value.substring(0, textarea.selectionStart)}${'\n'}${textarea.value.substring(textarea.selectionEnd)}`;
      ...
      case 'ArrowLeft': {
        if (getCursorPosition(textarea) !== 0) {
          textarea.setSelectionRange(getCursorPosition(textarea) - 1,
            getCursorPosition(textarea) - 1);
        }
        break;
      }
      case 'ArrowRight': {
      ...
      case 'Tab': {
        const TabSelEnd = textarea.selectionEnd;
        textarea.value = `${textarea.value.substring(0, textarea.selectionStart)}${'\t'}${textarea.value.substring(textarea.selectionEnd)}`;
        textarea.setSelectionRange(TabSelEnd + 1, TabSelEnd + 1);
        break;
      }
      case 'Space': {
        const SpaceSelEnd = textarea.selectionEnd;
        textarea.value = `${textarea.value.substring(0, textarea.selectionStart)}${' '}${textarea.value.substring(textarea.selectionEnd)}`;
  ...
  if (id === 'CapsLock' && flag) {
    ...
    Object.keys(keys).forEach((key) => {
      document.getElementById(key).classList.add('light');
    });
    light = true;
  } else if (id === 'MetaLeft' && light === true) {
    Object.keys(keys).forEach((key) => {
      document.getElementById(key).classList.remove('light');
    });
    light = false;
  }
  if ((id === 'ShiftRight' || id === 'ShiftLeft') && caps === true) {
    ...
    Object.keys(keys).forEach((key) => {
      if (keys[key].inner && count < 14) {
        document.getElementById(key).innerHTML = keys[key][lang][Math.abs(index)];
        count += 1;
      } else document.getElementById(key).innerHTML = keys[key][lang][Math.abs(index - 1)];
    });
  } else if (id === 'ShiftRight' || id === 'ShiftLeft') {
    index = shift ? index : Math.abs(index - 1);
    shift = true;
    Object.keys(keys).forEach((key) => {
      document.getElementById(key).innerHTML = keys[key][lang][index];
    });
...
function toUp(id) {
  ...
  if (id === 'CapsLock') {
    ...
    Object.keys(keys).forEach((key) => {
      if (keys[key].inner && count < 14) {
        document.getElementById(key).innerHTML = keys[key][lang][Math.abs(index - 1)];
        count += 1;
      } else document.getElementById(key).innerHTML = keys[key][lang][Math.abs(index)];
    });
  } else if (id === 'ShiftRight' || id === 'ShiftLeft') {
    shift = false;
    index = Math.abs(index - 1);
...
function KeyboardMove(property) {
  const shiftX = property.clientX - keyboard.getBoundingClientRect().left;
  const shiftY = property.clientY - keyboard.getBoundingClientRect().top;
  ...
  function moveAt(pageX, pageY) {
    ...
    if (
      ((pageY - shiftY + keyboard.clientHeight) < window.innerHeight)
           && ((pageY - shiftY) > 0)
    ) {
  ...
  moveAt(property.pageX, property.pageY);
